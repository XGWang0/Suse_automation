...
sub _read_partitions
{
	...
	chomp($abuildsize);
	chomp($bootsize);
	if ($args->{'to_arch'} =~ /ppc/i) {
		$bootpart="/dev/" . `ls -l \$(grep ^boot /etc/lilo.conf | cut -d \" \" -f3) \| awk -F/ {'print \$NF'}`;
		$bootpart=`fdisk  -l|sed -n '/PPC PReP/{s/ .*//;p;q}'` if ($args->{'to_arch'} =~ /ppc64le/i);
	} else {
...
# TODO: since when it is patterns and not RPM groups ?
	...
	if($args->{'setup_bridge'}) {
		...
		print $f "
		<script>
		  <filename>zzz_setup_bridge_ports</filename>
		  <interpreter>shell</interpreter>
		  <source>
		  for br in `ls /etc/sysconfig/network/ifcfg-br*`
		  do
			  interface_mac=`grep -o '..:..:..:..:..:..' \$br`
			  interface_name=`ip a|awk 'IGNORECASE=1{if(/'\$interface_mac'/){print a;exit};a=\$2;gsub(/:/,\"\",a)}'`
			  sed -i \"s/eth-id-.*/\$interface_name'/ \" \$br
		  done
		  </source> 
		</script>";
	}

	print $f "           </post-scripts>\n";
...
$postcmd		 
	...
	unless ($args->{'upgrade'}) {	
		...
		my $router_dev = `route -n|awk '{if(\$4~/[UG][UG]/){print \$8;exit}}'`;
		chomp $router_dev ;
		my $mac = `cat /sys/class/net/$router_dev/address`;
		chomp $mac;
...
sub _print_profile_partitions
{
	...
	unless ($args->{'upgrade'}) { #partitioning unless upgrade
		...
		unless ($args->{'virthosttype'} or $args->{'newvm'}) {
			...
			$drives->{$bootid}->{$bootnum}='NULL' if defined $bootid and ($args->{'to_arch'} =~ /ppc/i);
