...
system "scp -o StrictHostKeyChecking=no -i /usr/share/qa/keys/id_rsa \"$tmpfile\" $ruser\@$rhost:$rbase/" and die "Cannot SCP";
unlink $tmpfile or warn "Cannot unlink $tmpfile : $!";

my $cmd="ssh $ruser\@$rhost -i /usr/share/qa/keys/id_rsa /usr/share/qa/tools/qa_db_report.pl '".(join ' ',map {(/ / ? '"'.$_.'"' : $_)} @ARGV)."'";
&log(LOG_INFO,$cmd);
