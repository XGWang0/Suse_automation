...
my $testreports_url = $qaconf{testreports_url};
...
while( my $parser = readdir RESULTS) {
	...
	while ( my $tcf = $src->testsuite_list_next() )
	{
		...
		unless( $submission_id or $db_common::nodb )
		{
			...
			$submission_id = $dst->submission_create($args{'type'},$tester_id,$host_id,$args{'comment'},$arch_id,$product_id,$release_id,$config_id,$hwinfo_id,($args{'build_nr'} ? $args{'build_nr'} : undef));
...
sub get_patch_details_new # issuer_id, issue_id, request_id
{
	...
	my $rpmurl = "$testreports_url$issuer_id:Maintenance:$issue_id:$request_id/packages-list.txt";
	unless( open LIST, '-|', "curl -Sfs $rpmurl" )	{
		&log(LOG_CRIT,"Cannot open '$rpmurl':$!");
		return ();
	}
	my %released_rpms;
	while( my $row=<LIST> ) {
