	...
	<?php foreach ($machines as $machine): ?>
	   <tr
		...
		>
			...
			<td><?php echo($machine->get_status_string()); if ($machine->get_tools_out_of_date()) echo('<img src="images/exclamation_yellow.png" alt="Tools out of date!" title="Tools out of date(v'.$machine->get_tools_version().')!" width="20" style="float:right; padding-left: 3px;"></img>'); if ($machine->get_devel_tools()) echo('<img src="images/gear-cog_blue.png" alt="Devel Tools" title="Devel Tools" width="20" style="float:right; padding-left: 3px;"></img>'); ?></td>
		<?php
			foreach ($fields_list as $key=>$value){
				$fname = "get_".$key;
				$res = $machine->$fname();
