	...
	<?php foreach ($machines as $machine): ?>
	   <tr
		...
		>
			...
			<td><?php echo($machine->get_status_string()); if ($machine->get_tools_out_of_date()) echo('<a href="index.php?go=send_job&a_machines[]='.$machine->get_id().'&filename[]='.XML_DIR.'/Update_Tools.xml&submit=1"><img src="images/exclamation_yellow.png" alt="Tools out of date!" title="Tools outdated (v'.trim($machine->get_tools_version()).'). Update?" width="20" style="float:right; padding-left: 3px;"></img></a>'); if ($machine->get_devel_tools()) echo('<img src="images/gear-cog_blue.png" alt="Devel Tools" title="Devel Tools" width="20" style="float:right; padding-left: 3px;"></img>'); ?></td>
		<?php
			foreach ($fields_list as $key=>$value){
				$fname = "get_".$key;
				$res = $machine->$fname();
