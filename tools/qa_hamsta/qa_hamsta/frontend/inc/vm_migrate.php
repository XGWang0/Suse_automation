	...
	if (count($errors)==0) {
		...
		foreach ($machines as $machine) {
			if (!$machine->send_job($migratejobfile)) {
				...
				Log::create($machine->get_id(), $user->getLogin (), 'VM-MIGRATION', "of domain $domain_name to remote host with IP $migrateeIP and live migration option as " . ($livemigration ? "yes" : "no") . "has been processed.");
			}
		}
