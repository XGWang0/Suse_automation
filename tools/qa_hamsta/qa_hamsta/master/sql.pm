...
{	
	my $machine_list = $dbc->matrix_query('SELECT machine_status_id,machine_id FROM machine WHERE ip=? ORDER BY machine_id DESC',$_[0]);
	for my $machine (@$machine_list) {
		my ($machine_status_id,$machine_id) = @$machine;
		return $machine_id if $machine_status_id == 1;
	}
...
sub machine_get_status_by_ip($) # IP address
{	return $dbc->scalar_query('SELECT machine_status_id FROM machine WHERE ip=?',$_[0]);	}
...
sub machine_set_all_unknown(){
     ...
     $dbc->update_query('UPDATE machine SET busy=0');
}
...
{	return $dbc->scalar_query('SELECT job_status_id FROM job WHERE job_id=?',$_[0]);	}
...
sub job_insert($$$$$$$) # short_name, xml_file, description, user_id, job_status_id, aimed_host)
{	return $dbc->insert_query('INSERT INTO job(short_name,xml_file,description,user_id,job_status_id,aimed_host) VALUES(?,?,?,?,?,?)',@_);	}
...
sub job_get_info($) # job_id
{	return $dbc->matrix_query('SELECT p.job_part_id,r.mm_role,m.name,m.ip,p.job_part_on_machine_id FROM job j LEFT JOIN job_on_machine k USING(job_id) LEFT JOIN job_part_on_machine p USING(job_on_machine_id) LEFT JOIN mm_role r USING(mm_role_id) LEFT JOIN machine m USING(machine_id) WHERE j.job_id = ? ORDER BY p.job_part_id DESC',$_[0]); }
...
sub job_part_insert($) # job_id
{   return $dbc->insert_query('INSERT INTO job_part(job_id) VALUES(?)', $_[0]); }

sub job_part_delete_by_job_id($) # job_id
...
{
    return $dbc->scalar_query ('SELECT user_id FROM user WHERE email = ? limit 1', $_[0]);
}

sub user_get_email_by_id($) # id
{
    return $dbc->scalar_query ('SELECT email FROM user WHERE user_id = ?', $_[0]);
}
...
sub log_insert($$$$$$$) # machine_id, job_part_on_machine_id, log_time, log_type, log_user, log_what, log_text
{	return $dbc->insert_query('INSERT INTO `log`(machine_id,job_part_on_machine_id,log_time,log_type,log_user,log_what,log_text) VALUES(?,?,?,?,?,?,?)',@_);	}

sub log_get_by_job_part_on_machine_id($)
{	return $dbc->matrix_query('SELECT log_time,log_type,log_user,log_what,log_text FROM `log` WHERE job_part_on_machine_id = ?', $_[0]) ; }
