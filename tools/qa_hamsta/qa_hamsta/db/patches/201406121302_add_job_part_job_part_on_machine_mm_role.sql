...
-- Change table log
ALTER TABLE `log` ADD COLUMN `job_part_on_machine_id` INT NOT NULL DEFAULT 0;
...
DELETE FROM `log` WHERE job_on_machine_id IS NULL;
ALTER TABLE `log` 
    DROP FOREIGN KEY `log_ibfk_2`,     
    DROP COLUMN `job_on_machine_id`, 
    MODIFY COLUMN job_part_on_machine_id INT NOT NULL,
...
UPDATE `job` SET `timestamp` = (SELECT `timestamp` from `job_on_machine` WHERE `job`.job_id = `job_on_machine`.job_id);
