...
function tsDraw(p_id, p_table)
{
	...
	else
	{
		...
		for (i = 0; i < len; i++)
		{
			...
			var spc=clone.className.indexOf(' ');
			if( spc>0 )
				clone.className = classes[j++]+clone.className.substring(spc);
			else
...
function toggleTable(id)
{
	...
	var cls='shrank ';
	var idx = e.className.indexOf(cls);
...
//	var cols = thead.getElementsByTagName('th');
	...
	// If the table is shrank already (i.e. we are now expanding it)
	if( idx>=0 )
	{
		e.className = e.className.substring(cls.length);

		///// All of this code can be removed, by properly declaring "shranks" in style.css /////
//		for (i = 0; i < rows.length; i++)
//		{
...
//		tbody.removeChild(rows[rows.length - 1]);
	...
	// If we are now shrinking the table
	else
	{
...
//		newtd.colSpan='' + cols.length + '';
//		newtd.innerHTML='This table has been shrunk! Click the arrows above to expand it.';
