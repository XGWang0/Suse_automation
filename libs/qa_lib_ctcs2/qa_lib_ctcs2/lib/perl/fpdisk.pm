...
sub fdisk_type {
	...
	system("which pdisk >/dev/null 2>&1");
	# If no pdisk, take fdisk or fdasd(for IBM s390x DASD devices)
	if ($? != 0) {
		if (`uname -m` eq "s390x" || $device =~ /\/dev\/dasd[a-z]/) {
			return "fdasd";
		} else {
			return "fdisk";
		}
	} else {
        # pdisk is there, check for valid output
                if (`LANG=C pdisk -l 2>&1` =~ /No valid block/) {
                # on the OpenPOWER pdisk does not work
                        return "fdisk";
                }
        }
...
# and the special cases are getting out of hand.
...
# will result in bad things, don't do that.
sub get_disk_of {
	my $partition = shift;
	# Funky RAID style
...
# Get IBM s390x DASD disk info
sub get_disk_info_fdasd {
	my $disk = shift;
	my $partitions = 0;
	my ($heads, $sectors, $cylinders, $blocksize);
	my $disksize;
	open FDASD, "LANG=C fdasd -p $disk |";
	# parse fdasd output line by line
	my $flag = 0;
	while (<FDASD>) {
		if (/cylinders.*:\s*(\d+)/) {
			$cylinders = $1;
		} elsif (/tracks per cylinder.*:\s*(\d+)/) {
			$heads = $1;
		} elsif (/blocks per track.*:\s*(\d+)/) {
			$sectors = $1;
		} elsif (/bytes per block.*:\s*(\d+)/) {
			$blocksize = $1;
		}
		if (/Device\s*start\s*end\s*length/) {
			$flag = 1;
		}
		if ($flag == 1 && /\/dev\/dasd[a-z]+\d+/) {
			++$partitions;
		}
	}
	close FDASD;
...
sub get_disk_info {
	...
	if($type eq "fdisk") {
		...
		return(get_disk_info_fdasd($disk));
	} elsif ($type eq "pdisk-linux") {
