    ...
    def __init__(self):
        ...
        self.add_option("-t", "--test-type", action="store", type="string",
                        ...
                        help=("Set test type, gi|hu|gm is available"
                              ...
                              "\ngm represents Prj3 Guest Migration"
                              "\ngu represents Prj4 Guest Upgrade"))
        '''
        ...
        self.add_option("--sub_tst_mode", action="store", type="string",
                        dest="sub_test_mode",# choices=['miles','daily'],
                        help=("[milestone/daily], milestone means that using standard repo's package of milestone to execute test"
                              "daily means that using daily buiild repo's package to run"))        
        ...
        #Guest migration and vm-guest verfication test parameters
        group = optparse.OptionGroup(
            self,
        ...
        #Guest upgrade and vm-guest verfication test parameters
        group = optparse.OptionGroup(
            self,
            "Prj4:Guest Upgrade",
            "This tool is only for testing guest upgrade feature")
    
        self.add_option_group(group)
        group.add_option("--gu-host", action="store", type="string",
                        dest="gu_host_list",
                        help=("Input test machine[s] ip address or domain name which shows in hamsta server."
                              "Multiple ip address means that running test in parallel, "
                              "parallel number is count of machines"))
        group.add_option("--gu-host-product", action="store", type="string",
                        dest="gu_h_product_list",
                        help=("Input product version of host, supporting product : \"SLES-11-SP4-64.XEN"
                              "SLES-11-SP4-64.KVM SLES-12-SP0-64.XEN SLES-12-SP0-64.KVM SLES-12-SP1-64.XEN"
                              "SLES-12-SP1-64.KVM\", using comma as separator"))
        group.add_option("--gu-guest-product", action="store", type="string",
                        dest="gu_guest_prd",
                        help=("Guest product version as OS of vitual machine were installed on host machine."
                              "Product scope:\"nw-65,oes-11,oes-2,rhel-3,rhel-4,rhel-5,rhel-6,rhel-7,sled-10,"
                              "sled-11,sled-12,sles-10,sles-11,sles-12,sles-9,win-2k,win-2k12,win-2k12r2,"
                              "win-2k3,win-2k8,win-2k8r2,win-7,win-8,win-8.1,win-vista,win-xp\","
                              "Using comma as separator"))
        group.add_option("--gu-guest-upg-traget", action="store", type="string",
                         dest="gu_guest_upg_prd",
                         help=("The parameter means the product which all guest will upgrade to"))
...
def main():
    ...
    if options.test_type == "gi":
        import jenkins_run_milestone_prj1 as jrm
    elif options.test_type == "hu":
        import jenkins_run_milestone_prj2 as jrm
    elif options.test_type == "gm":
        import jenkins_run_milestone_prj3 as jrm
    elif options.test_type == "gu":
        import jenkins_run_milestone_prj4 as jrm

    #Initial environment
    jrm.AllStaticFuncs.cleanJosnFIle()

    #Instance for multiple process    
    mpr = jrm.MultipleProcessRun(options)
    mpr.startRun()
    ...
    jrm.AllStaticFuncs.compressFile(jrm.AllStaticFuncs.getBuildPath())
    
    #Verify project result and mark status
    if mpr.getMulPoolStatus()["status"] is True:
        exit_code = 0
    else:
        jrm.LOGGER.warn(mpr.getMulPoolStatus()["info"])
        exit_code = 5
    sys.exit(exit_code)
