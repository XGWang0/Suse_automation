...
from jenkins_run_devel_prj2 import * 

class HostMigration(HostMigration, object):
    ...
    def execCMDForGenerateCase(self, phase="phase2", cmd="", timeout=1800):
        ...
        if self.status:
            ...
            LOGGER.warn("Last phase failure, skip test run script generation.")

    def generateTestRunForDaily(self, timeout=1800):
        """Function which update host by hamsta API
        """
        ...
        cmd_generate_tr = (self.feed_hamsta +  " -x "
                       ...
                       "-m %(virt_std)s -v %(virt_type)s -b %(org_prd)s -u %(dest_prd)s -i \'%(guest_prd)s\' -l %(upd_repo)s \" "
        ...
        self.execCMDForGenerateCase(cmd=cmd_generate_tr, timeout=timeout)

    def generateTestRunForMiles(self, timeout=1800):
        # Get guest regular expression pattern
        guest_prd = self.getGuestFilter()
        cmd_generate_tr = (self.feed_hamsta +  " -x "
               "\"/usr/share/qa/tools/_generate_vh-update_tests.sh "
    ...
    def verifyGuestForDaily(self, timeout=10000):
        ...
        if self.status:
            ...
            LOGGER.warn("Last phase failure, skip guest verification.")

    def verifyGuestForMiles(self, timeout=10000):
        """Function which verifys result of host migration.
        Thru invoking hamsta cmd to do this operation.
        """
        if self.status:
    ...
    def updateRPMFromDaily(self, phase="Phase0", job_sketch="Upgrade Virt RPM", timeout=36000):
        ...
        if self.status:
            ...
            LOGGER.warn("Last phase failure, skip milestone virt rpm updating.")
    ...
    def makeEffect2HostUpgrade(self, phase="Phase0", flag=True):
        if self.status:
            ...
            pass
    '''
def migrateHost(org_prd, dest_prd, param, queue=None, sub_test_mode=""):
    """Externel function, only for warp migration host function
    """
    def _migrateHostDaily(hm_inst):
        if hm_inst.status:
            ...
            hm_inst.generateTestRunForDaily()
            ...
            hm_inst.updateRPMFromDaily(phase="Phase8",
                                       job_sketch="Upgrade Virt RPM For %s.%s" %(hm_inst.dest_prd,
                                                                                     hm_inst.virt_type))
            # Verification Result
            hm_inst.verifyGuestForDaily()
    ...
    def _migrateHostMiles(hm_inst):
        if hm_inst.status:
            ...
            # Generate test run script
            hm_inst.generateTestRunForMiles()
            ...
            # Verification Result
            hm_inst.verifyGuestForMiles()

            hm_inst.releaseHost()

    vir_opt = HostMigration(org_prd, dest_prd, param, queue)
    LOGGER.info("Host Migration Milestone Test from [%s] to [%s] starts to run on host [%s] now" %(org_prd,
                                                                                    dest_prd,
                                                                                    vir_opt.host))
    if sub_test_mode == "milestone":
        _migrateHostMiles(vir_opt)
    else:
        _migrateHostDaily(vir_opt)

    vir_opt.writeLog2File()
    LOGGER.info("Host Migration Milestone Test from [%s] to [%s] finished" %(org_prd, dest_prd))
...
class MultipleProcessRun(MultipleProcessRun, object):
    ...
    def _huMultipleTask(self):
        ...
        for test_suite in self.scenarios:
            ...
            #migrateHost(ord_prd, upg_prd, self.param, self.queue, self.sub_test_mode)

            self.result.append([ord_prd+upg_prd,
                self.pool.apply_async(migrateHost,
                                      (ord_prd, upg_prd, self.param, self.queue, self.sub_test_mode))])
