...
print_usage()
{
	echo "Usage: $0 --help -h -help | -y -M -T -o -r -p -c -t -n -m -e -x -s -d -a -i -z -g -P -F -N -I -H -D -S -C -R -l -k"
	echo "Options: "
	...
	echo " -k shutdown the vm after test if -g is set"
	echo " -P <settingsFile>"
	echo " -Q <sourcesFile>"
	echo " -N <nameAddition>"
	echo " -F <fullVmName>"
...
print_full_usage()
{
	...
	echo
	echo "Usage: $0 --help -h -help | -y -M -T -o -r -p -c -t -n -m -e -x -s -d -a -i -z -g -P -N -F -I -H -D -S -k"
	echo
	echo "Options: "
	echo
	echo " -h,-help,--help"
	...
	echo
	echo " -k"
	echo "        - Provide this option if you want to shut down the VM after it is done installing"
	echo "        - DEFAULT: Leave the VM in running state"
	echo 
	echo " -P <settingsFile>"
...
while getopts "y:M:T:o:r:p:c:Ct:n:m:e:x:s:d:a:i:P:Q:N:F:H:I:D:L:S:zgZ:b:RlA:f:O:E:k" OPTIONS
do
	case $OPTIONS in
		y) xenHostIp="$OPTARG";;
...
[ "$bridge" != "" ] && if ! ifconfig2ip dev "$bridge" > /dev/null 2>&1 
then
...
# else in fv -> move it during pxe setup
if [ "$virtType" == "fv" ]
then
...
#sleep 10 to let DHCP update logfile
...
then
	...
	if [ "$keepRunning" == "NO" -a "$installResult" == "PASS" ]
	then
		echo "We are going to shutdown the installed VM..."
		virsh destroy $machineName
		if [ $? -ne 0 ]
		then
			tmpError="Could not shut down the new install"
			echo "ERROR - $tmpError"
			echo "ERROR - $tmpError" >&2
			echo "    ***** INSTALL ERRORED *****"
			popd > /dev/null; exit $rERROR
		...
		echo echo "We are not going to shutdown the installed VM"
	fi
fi
