...
############################################################################################
#=== Verify before and after guest upgrade, the virtual host can administrate vms correctly#
############################################################################################
...
function usage() {
	echo
	echo "Usage: $0 -p PRODUCT_UPGRADE -r PRODUCT_UPGRADE_REPO [-g GUEST_LIST]"
...
function verify_guest_up_via_ssh() {
    ...
    while [ $tryTimer -lt $timeout ];do
        ...
        if [ $ret -ne 0 ];then
            ...
            echo "The vm $vm is up via ssh connection check after ${tryTimer}s."
            break
        fi
    done

    echo "Debug info: /tmp/virt-install_screenshot/ content is:"
    ls /tmp/virt-install_screenshot/

    if [ $ret -eq 0 ];then
        rm /tmp/virt-install_screenshot/${vm}*
        return 0
    else
        echo "Error: The vm is not up until timeout ${timeout}s." >&2
        return 1
    fi
...
#test flow
backupRootDir=/tmp/prj4_guest_upgrade/vm_backup
...
#handle special guest sles10sp4 on kvm host to use correct network module virtio
change_sles10sp4_guest_network_to_virtio_on_kvm_host
...
#loop to do upgrade test on all guests
do_full_guest_upgrade_test "$PRODUCT_UPGRADE" "$PRODUCT_UPGRADE_REPO"
((exitCode+=$?))
