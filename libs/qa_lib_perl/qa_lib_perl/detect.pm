...
our %qadb_products=( map {lc($_)=>$_} (
	...
	(map {("SLES-$_","SLED-$_","SLERT-$_")} qw(10 10-SP1 10-SP2 10-SP3 11 11-SP1 11-SP2 11-SP3 11-SP4 12-SP0 12-SP1 12-SP2 12-SP3 12-SP4)),
...
our %qadb_releases=( map {lc($_)=>$_} (
	...
	qw(GM GMC GMC2 GMC3 GMC4 GMC5 internal maintained RC6 beta6 beta7 beta8 beta9 beta10)
	));
...
our $arch_re="(i\\d86|ppc(64)?(le)?|s390x?|ia64|x86[_-]64)";
...
{
	open(SUSE_Rele,"/etc/SuSE-release") or return ('','','','','');
	my @suse_re = <SUSE_Rele>;
	close(SUSE_Rele);
	my($version,$patchlvl);
	for my $line (@suse_re){
		$version = $1 if $line =~ /^version\s*=\s*(\d+)/i;
		$patchlvl = $1 if $line =~ /^patchlevel\s*=\s*(\d+)/i;
	}
	return ('',defined($version)?$version:'',defined($patchlvl)?$patchlvl:'','','');
	return ('',$version,$patchlvl,'','');
}
...
sub guess_product_from_url # URL
{
	...
	if( 
		...
		$url =~ /SLE-(\d+)-(?:Server|Desktop|sp[\d]+)/i
	)
	{
		$version = $1;
		if( $url =~ /SLES for SAP/ )
...
sub detect_product
{
	...
	push @data, [ 'etc-suse-release',&parse_suse_release(),''];
	...
	$release = 'build'.$build_nr if $build_nr;
